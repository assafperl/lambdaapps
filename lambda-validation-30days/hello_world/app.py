...
def get_google_sheet():
    ...
    service = build('sheets', 'v4', credentials=credentials)
    sheet = service.spreadsheets()
    results = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range="every30days!A1:G50").execute()
    column_names = results['values'].pop(0)
    googlesheetdf = pd.DataFrame(results['values'], columns=column_names)
...
def loop_the_sheet():
    ...
    for email_list in googlesheetdf['Owner email'].unique():
        ...
        message['from'] = sender
        message['subject'] = "Data Validations(30days) - ***soft launch*** " + str(dateTimeObj)
        counter = 1
