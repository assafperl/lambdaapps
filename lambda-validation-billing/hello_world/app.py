...
def get_google_sheet_contacts():
    ...
    results = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range="soft-launch!A1:D50").execute()
...
def get_google_sheet():
    ...
    service = build('sheets', 'v4', credentials=credentials)
    sheet = service.spreadsheets()
    results = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range="Billing!A1:K50").execute()
    column_names = results['values'].pop(0)
    googlesheetdf = pd.DataFrame(results['values'], columns=column_names)
...
def loop_the_sheet():
    ...
    for email_list in googlesheetdf['Owner email'].unique():
        ...
        group = group.sort_values('Importance',ascending=True)
        ...
        message['from'] = sender
        message['subject'] = "Billing Data Validations - ***soft launch*** " + str(dateTimeObj)
        counter = 1
        for index, row in group.iterrows():
            ...
            if attdf.shape[0] != 0 and attdf.values[0][0] != 'Success':
                ...
                message_text = '\n' + message_text + '\n' + "\u0332".join(row['Importance']) + '\n' + '\n' + '\t' + str(counter) + '. ' + str(attdf.shape[0]) + ' ' + row['Mail Sentence'] + '\n' + '\t' + 'Attached ' + destination + '\n' + '\n'
                counter = counter + 1
