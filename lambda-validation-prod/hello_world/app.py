...
def get_google_sheet():
    ...
    service = build('sheets', 'v4', credentials=credentials)
    sheet = service.spreadsheets()
    results = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range="Sheet1!A1:I100").execute()
    column_names = results['values'].pop(0)
    googlesheetdf = pd.DataFrame(results['values'], columns=column_names)
    googlesheetdf['prod_script'] = googlesheetdf['prod_script'].str.replace('\n', ' ')
    googlesheetdf['prod_script'] = googlesheetdf['prod_script'].str.replace('\r', '')
    return googlesheetdf
