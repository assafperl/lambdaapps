...
def refresh_tableau_workbooks_datasources_by_tag():
    tableaudict = get_secret_value('tableau-dev')
    name = json.loads(tableaudict['SecretString'])['name']
    token = json.loads(tableaudict['SecretString'])['token']
    ...
    with server.auth.sign_in(tableau_auth):
        ...
        for datasource in all_datasources:
            if datasource.tags:
                if 'dev' in [tag.lower() for tag in datasource.tags]:
        ...
        for workbook in all_workbooks:
            if workbook.tags:
                if 'dev' in [tag.lower() for tag in workbook.tags]:
